...
public class CatIsland {
    ...
    public static void island4(Island islandFour) throws CatInWaterException {
        ...
        Cat newCat = new Cat("Soham", islandFour, 3, 3, Color.BROWN);
    ...
    public static void island5Recursive(Cat cat, Tile[][] tiles, Tile prev) throws CatInWaterException {
        ...
        // Move Down
        if (row + 1 < tiles.length && tiles[row + 1][col].isLand() && tiles[row + 1][col] != prev) {
            cat.moveDown();
            island5Recursive(cat, tiles, tiles[row][col]);
            return;
        }
        ...
        // Move Right
        if (col + 1 < tiles[0].length && tiles[row][col + 1].isLand() && tiles[row][col + 1] != prev) {
            cat.moveRight();
            island5Recursive(cat, tiles, tiles[row][col]);
            return;
        }
